...
function Aside() {
  return (
    <h1>Aside</h1>
    // <aside
    //   className={`relative inset-y-0 z-10 flex flex-col flex-shrink-0 w-64 max-h-screen overflow-hidden transition-all transform bg-white border-green-100 border-r shadow-lg  lg:z-auto lg:static lg:shadow-none ${props.isSideBarOpen ? '' : 'w-20'}`}>
    //   <div
    //     className={`flex items-center justify-between flex-shrink-0 p-2 ${props.isSideBarOpen ? '' : 'lg:justify-center'}`}>
    ...
    //       className={`${props.isSideBarOpen ? 'max-[1024px]:relative' : 'hidden'} p-2 rounded-md lg:hidden`}>
    ...
    //           to="home"
    //           style={({ isActive }) =>
    //             isActive ? { color: 'white', backgroundColor: '#0F6938' } : undefined
    //           }
