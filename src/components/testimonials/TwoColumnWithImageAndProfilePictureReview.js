...
const QuoteContainer = tw.div`relative mt-10 lg:mt-20 text-secondary-100`;
const Quote = tw.blockquote`text-center lg:text-left text-sm sm:text-base lg:text-lg xl:text-xl`;
const CustomerInfo = tw.div`mt-6 flex flex-col sm:flex-row items-center justify-center lg:justify-start`;
const CustomerProfilePicture = tw.img`rounded-full w-20 h-20`;
const CustomerTextInfo = tw.div`text-center text-secondary-100 lg:text-left sm:ml-6 mt-2 sm:mt-0`;
const CustomerName = tw.h5`font-semibold text-secondary-100 text-xl lg:text-2xl xl:text-3xl`;
const CustomerTitle = tw.p`font-medium text-secondary-100`;

const QuotesLeft = tw(
  QuotesLeftIcon,
)`w-6 h-6 opacity-75 text-green-500 inline-block mr-1 -mt-3`;
const QuotesRight = tw(
  QuotesRightIcon,
)`w-6 h-6 opacity-75 text-green-500 inline-block ml-1 -mt-3`;
...
const DecoratorBlob2 = tw(
  SvgDecoratorBlob2,
)`absolute w-32 bottom-0 right-0 -z-10 text-pink-500 opacity-20 transform translate-x-2/3 translate-y-8`;
            ...
            <Testimonial>
              ...
              >
                {testimonials.map((testimonial) => (
                  <ImageAndControlContainer key={testimonial.customerName}>
              ...
              <TextContainer textOnLeft={textOnLeft}>
                ...
                >
                  {testimonials.map((testimonial) => (
                    <TestimonialText key={testimonial.customerName}>
                      <QuoteContainer>
                        <Quote>
                          <QuotesLeft />
                          {testimonial.quote}
...
function HeadingInfo({ subheading, heading, description, ...props }) {
  return (
    <div {...props}>
