    ...
    return(
        ...
        >
            <div className="flex justify-between">
                ...
                <button className="text-green-100 hover:text-red-900" onClick={() =>props.setAddModal(false)}>
            ...
            <form onSubmit={addProduct} className="flex flex-col">
                <div className="flex flex-row gap-x-2">
                    ...
                    <Input type="number" name="price" value={formData.price} placeholder="Price" onChange={handleInputChange}/>
                </div>
                <div className="flex flex-row gap-x-2">
                    <Input type="text" name="flavor" value={formData.flavor} placeholder="Flavor" onChange={handleInputChange}/>
                    <Input type="text" name="code" value={formData.code} placeholder="Code" onChange={handleInputChange}/>
                </div>
                <div className="flex flex-row gap-x-2">
                    <Input type="text" name="description" value={formData.description} placeholder="Description" onChange={handleInputChange}/>
                    <Input type="text" name="weight" value={formData.weight} placeholder="Weight" onChange={handleInputChange}/>
                </div>
                <div className="flex mt-5 mb-5 items-center justify-center w-full">
                    <label htmlFor="dropzone-file" className="flex flex-col bg-cover items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                        {imagePreview ? 
                        ...
                        <input onChange={imageUpload} id="dropzone-file" name="image" type="file" accept="image/*" className="hidden" />
                    </label>
                </div>
                <div className="flex gap-x-2">
                    <Select isLoading={isLoadingCategory} lists={category} label="Select Speciality"/>
                    <Select isLoading={false} lists={[{"name":"Available"}, {"name":"Out of Stock"}, {"name":"In production"}, {"name":"Inactive"}]} label="Select Status"/>
                </div>
                <br/>
                {isLoading ?
