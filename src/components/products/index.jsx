    ...
    return(
        <main className="flex-1 max-h-full p-5 overflow-hidden overflow-y-scroll">
            ...
            <div className="flex flex-col space-y-6 md:space-y-0 md:flex-row justify-between mr-8 mt-8 ml-12">
                <SearchInput placeholder="Search products" value={searchField} type="search" onChange={(event)=> setSearchField(event.target.value)}/>
                        ...
                        <div className="overflow-hidden border-b border-gray-200 rounded-md shadow-md">
                            <table className="mt-8 min-w-full overflow-x-scroll divide-y divide-gray-200">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200 flex-">
                                    { isLoading ? 
                                        <tr>
                                            <td colSpan="6" className="grid text-center py-4"><CircularProgress /></td>
                                        </tr>
                                        :      
                                            ...
                                            (filteredProducts.map((product) => (
                                                <tr key={product.id} className="transition-all hover:bg-gray-100 hover:shadow-lg">
                                                    ...
                                                    <td className="space-x-2 text-green-400 text-lg">
                                                        <button className="hover:text-gray-900"><FaEdit/></button>
                                                        <DeleteAlert id={product.id} reduxAction ={deleteProduct} endpoint="/products/delete/"/>
